...
const Questionnaire = () => {
  ...
  //runs only first time when we choose our device type , then never runs in future 
  useEffect(() => {
    if (initRef.current) return;    //it prevents infinite loop of re-rendering
  ...
  //(important)this above variable is having the data of questions based on the device type


  const handleNext = () => {
    if (currentStep === questions.length - 1) {//if the current step is the last step
      complete(); //complete the questionnaire
      navigate(answers.deviceType === 'laptop' ? '/results/laptops' : '/results/pc'); //navigate to the results page based on the device type
    } else {
        ...
        return (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <motion.button
              ...
              className={`p-6 rounded-2xl border text-center transition-all ${answers.deviceType === 'pc'
            ...
            <motion.button
              ...
              className={`p-6 rounded-2xl border text-center transition-all ${answers.deviceType === 'laptop'
                  ? 'border-accent bg-accent/5 shadow-sm'
                  : 'border-border bg-card hover:border-accent/30'
                }`}
            >
            ...
            <div className="grid grid-cols-3 gap-2">
              {[50000, 75000, 100000, 150000, 200000, 300000].map((amount) => (
                <button
                  ...
                  className={`py-2.5 px-3 rounded-xl text-sm font-medium transition-all ${answers.budget === amount
            ...
            {answers.budget && (
              ...
              >
                <p className="text-sm text-muted-foreground">
                  {/*  added heuristic-based budget tiers to set user expectations before generating recommendations.” */}
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 pt-24 pb-16 min-h-screen">
        ...
        <AnimatePresence mode="wait">
          <QuestionCard
            // here key is used to identify the current question or the unique key for each question.

            // these are the props that are passed to the QuestionCard component.
            //these are the children for questioncard.tsx which i have defined in the renderQuestion method.
            key={`${answers.deviceType}-${currentStep}`}
            step={currentStep}
            totalSteps={questions.length}
            question={questions[currentStep].question}
            description={questions[currentStep].description}
            onNext={handleNext}//this handlenext is defined above to check whether the next step is last or not , if yes then go for results
            onPrev={handlePrev}//"" "" "" "" ""to check whether the previous step is first or not , if yes then go back to the previous step.
            canProgress={canProgress()}//this canprogress is defined above to check whether the next step is valid or not.
            isFirst={currentStep === 0}//this is used to check if the current step is first or not.
            isLast={currentStep === questions.length - 1}//this is used to check if the current step is last or not.
          >
            {renderQuestion()}
          </QuestionCard>
        </AnimatePresence>
