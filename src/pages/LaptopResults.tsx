...
const LaptopResults = () => {
  ...
  useEffect(() => {
    ...
    //This is the main controller function of the page.
    // This function is responsible for fetching the laptop recommendations from the backend.
    // It first checks if the recommendations are already cached.
    // If not, it fetches the recommendations from the backend.
    // It then fetches the live prices of the laptops from the backend.
    // Finally, it sets the recommendations and insights and updates the loading state.
    async function fetchLaptops() {
      ...
      try {
        ...
        //here just below we are defining the budget ceiling.
        //the budgetCeiling is the maximum price that the laptop can have. it is calculated by multiplying the budget by 1.15
        //the 1.15 is the percentage of the budget that can be used to buy a laptop.
        //it is calculated by taking the average of the prices of the laptops.

        const budgetCeiling = Math.round((answers.budget || 100000) * 1.15);
        let validRecs: LaptopRecommendation[] = [];
        let attempts = 0;
        const maxAttempts = 3;
        //here we are calling the fetchGeminiLaptops function to fetch the laptop recommendations from the backend.
        //the fetchGeminiLaptops function is defined in the file geminiApi.ts and is used to fetch the laptop recommendations from the backend.
        //this function is called only once for each user.
        //here maxAttempts is for limiting the number of attempts to fetch the laptop recommendations from the backend.
        //here validRecs.length is for limiting the number of recommendations to fetch from the backend.
        ...
        while (validRecs.length < 3 && attempts < maxAttempts) {
          ...
          // Fetch live prices using livePricingApi.ts file
          //here we are apart from fetching laptop recommendations from backend we are also fetching live prices of the laptops from the backend.
          //the live prices are fetched using the fetchLiveAmazonPrice function defined in the file livePricingApi.ts
          //the first parameter is the search query of the laptop.
          //the second parameter is the brand of the laptop.
          //the third parameter is the model of the laptop.
          //the fourth parameter is the price of the laptop.
        ...
        //****in this step we are taking the first 3 recommendations from the validRecs array.
        const finalRecs = validRecs.slice(0, 3);
        //here we are caching the recommendations in the apiCache object.
        apiCache.set(answers, finalRecs);

        setRecommendations(finalRecs);
        //here we are generating the insights for the recommendations.
        //here setInsights is a usestate hook which is used to update the insights.
        //generateLaptopInsights is a function which is used to generate the insights.
        //here insight is basically the summary of the recommendations. 
        //summary is like example : "The MSI Cyborg is a great laptop for gaming and content creation. It has a great processor and graphics card, and it is a great value for the price."  
        //here insight is the text which is displayed below the recommendations. in the ui.
        setInsights(generateLaptopInsights(finalRecs, answers));
        //here we are setting the expandedDeals state to the first recommendation's id.
    ...
    //here fetchLaptops is a usestate hook which is used to update the recommendations.
   // this function is called only once when the component is mounted and when the hasValidAnswers state is changed.  
    fetchLaptops();
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 pt-24 pb-20">
        ...
        {/* this cards are showing the recommended laptops if any of the laptop is not having the price in the price list then it will not be shown  */}
            ...
            return (
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  {/* Spec pills like : cpu,gpu ,ram ,disk storage etc these spec should be according to the recommendation engine   */}
                  ...
                  <div className="mt-auto pt-4 border-t border-border">
                    ...
                    <div className="flex gap-2 mb-2 relative">
                      <Button
                        ...
                        // this handleAddLaptopToBucket is defined in the  main.tsx file this function adds the laptop to the bucket in the  useQueryStore   it uses the  setQuery to add the laptop to the bucket . the laptop bucket is  shown  in the query bucket page 
